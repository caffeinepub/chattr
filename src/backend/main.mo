...
actor {
    ...
    var reactions : OrderedMap.Map<Text, List.List<Reaction>> = textMap.empty();
    ...
    public shared ({ caller }) func deleteChatroomWithPassword(chatroomId : Nat, password : Text) : async () {
        ...
        switch (natMap.get(chatrooms, chatroomId)) {
            ...
            case (?_chatroom) {
                ...
                for ((messageId, _messageReactions) in textMap.entries(reactions)) {
                    ...
                    if (messageExistsInChatroom) {
                        updatedReactions := textMap.delete(updatedReactions, messageId);
                    };
                };
    ...
    public shared ({ caller }) func createChatroom(topic : Text, description : Text, mediaUrl : Text, mediaType : Text, category : Text) : async Nat {
        ...
        let firstMessage : Message = {
            id = formatMessageId(nextMessageId);
    ...
    public shared ({ caller }) func sendMessage(content : Text, sender : Text, chatroomId : Nat, mediaUrl : ?Text, mediaType : ?Text, avatarUrl : ?Text, senderId : Text, replyToMessageId : ?Text) : async () {
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
    ...
    public shared ({ caller }) func pinVideo(chatroomId : Nat, messageId : Text) : async () {
        if (not AccessControl.hasPermission(accessControlState, caller, #admin)) {
    ...
    public query func getPinnedVideo(chatroomId : Nat) : async ?Text {
        switch (natMap.get(chatrooms, chatroomId)) {
            case (null) { null };
    ...
    public shared ({ caller }) func addReaction(messageId : Text, emoji : Text, userId : Text) : async () {
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
            Debug.trap("Unauthorized: Only users can add reactions");
        };

        let messageReactions = switch (textMap.get(reactions, messageId)) {
        ...
        if (not found) {
            ...
            reactions := textMap.put(reactions, messageId, List.push(newReaction, messageReactions));
            ...
            reactions := textMap.put(reactions, messageId, List.reverse(updatedReactions));
    ...
    public shared ({ caller }) func removeReaction(messageId : Text, emoji : Text, userId : Text) : async () {
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
        ...
        reactions := textMap.put(reactions, messageId, updatedReactions);
    };

    public query func getReactions(messageId : Text) : async [Reaction] {
        switch (textMap.get(reactions, messageId)) {
            case (null) { [] };
        ...
        switch (natMap.get(messages, chatroomId)) {
            ...
            case (?chatroomMessages) {
                ...
                Array.map<Message, MessageWithReactions>(
                    sortedWithReactions,
                    func(message) {
                        let messageReactions = switch (textMap.get(reactions, message.id)) {
                            case (null) { List.nil<Reaction>() };
                            case (?existingReactions) { existingReactions };
                        };
    ...
    public query func getReplyPreview(chatroomId : Nat, messageId : Text) : async ?ReplyPreview {
        switch (natMap.get(messages, chatroomId)) {
            case (null) { null };
            case (?chatroomMessages) {
    ...
    public query func getReplies(chatroomId : Nat, parentMessageId : Text) : async [Message] {
        switch (natMap.get(messages, chatroomId)) {
            case (null) { [] };
            case (?chatroomMessages) {
